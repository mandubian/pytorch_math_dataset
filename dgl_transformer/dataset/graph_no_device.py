...
class GraphPoolNoDevice:
    ...
    def beam(self, src_buf, start_sym, max_len, k):
        '''
        ...
        for src_sample, n, n_ee, n_ed, n_dd in zip(src_buf, src_lens, num_edges['ee'], num_edges['ed'], num_edges['dd']):
            for _ in range(k):
                src.append(th.tensor(src_sample, dtype=th.long))
                src_pos.append(th.arange(n, dtype=th.long))
                enc_ids.append(th.arange(n_nodes, n_nodes + n, dtype=th.long))
                n_nodes += n
                e2e_eids.append(th.arange(n_edges, n_edges + n_ee, dtype=th.long))
                n_edges += n_ee
                tgt_seq = th.zeros(max_len, dtype=th.long)
                tgt_seq[0] = start_sym
                tgt.append(tgt_seq)
                tgt_pos.append(th.arange(max_len, dtype=th.long))

                dec_ids.append(th.arange(n_nodes, n_nodes + max_len, dtype=th.long))
                n_nodes += max_len
                e2d_eids.append(th.arange(n_edges, n_edges + n_ed, dtype=th.long))
                n_edges += n_ed
                d2d_eids.append(th.arange(n_edges, n_edges + n_dd, dtype=th.long))
                n_edges += n_dd
    ...
    def __call__(self, src_buf, tgt_buf):
        '''
        ...
        for src_sample, tgt_sample, n, m, n_ee, n_ed, n_dd in zip(src_buf, tgt_buf, src_lens, tgt_lens, num_edges['ee'], num_edges['ed'], num_edges['dd']):
            ...
            tgt_y.append(th.tensor(tgt_sample[1:], dtype=th.long))
            src_pos.append(th.arange(n, dtype=th.long))
            tgt_pos.append(th.arange(m, dtype=th.long))
            enc_ids.append(th.arange(n_nodes, n_nodes + n, dtype=th.long))
            n_nodes += n
            dec_ids.append(th.arange(n_nodes, n_nodes + m, dtype=th.long))
            n_nodes += m
            e2e_eids.append(th.arange(n_edges, n_edges + n_ee, dtype=th.long))
            n_edges += n_ee
            e2d_eids.append(th.arange(n_edges, n_edges + n_ed, dtype=th.long))
